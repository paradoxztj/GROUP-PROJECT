...
def main():
    ...
    timestamp = time.strftime("%Y%m%d_%H%M%S")  # Generate the current timestamp string for file naming
    image_path = f"raw_image_{timestamp}.jpg"  # Define the save path for the original image

    image_captured = True  # Flag variable, default is photo taken successfully

    try:
        subprocess.run([                        # Call the libcamera-still command to take a photo
            "libcamera-still",                  # Take a photo command
            "-o", image_path,                   # Output the image path
            "-t", "1000",                       # Wait for 1 second before taking the photo
            "--nopreview",                      # Do not display the preview window
            "--width", "1456",                  # Set the image width
            "--height", "1088"                  # Set the image height
        ], check=True)                          # Enable exception checking, an error will be thrown if the photo fails
        print(f"Image captured: {image_path}")  # Output the image save path after successfully taking the photo
    except subprocess.CalledProcessError as e:  # Catch the exception when the photo fails
        print(f"Camera capture failed: {e}")    # Output the error message
    ...
    if frame is not None:
        ...
        counts, gps_data, object_positions, detections = analyze_frame(
        frame, drone_lat, drone_lon, drone_alt, roll, pitch, yaw)
